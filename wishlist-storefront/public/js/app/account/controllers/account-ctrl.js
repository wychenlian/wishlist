...
angular.module('ds.account')
    ...
    .controller('AccountCtrl', ['$scope', 'addresses', 'account', 'orders', 'wishlistItems','OrderListSvc', 'AccountSvc', '$uibModal', 'GlobalData', '$translate','WishlistSvc',

        function ($scope, addresses, account, orders,wishlistItems, OrderListSvc, AccountSvc, $uibModal, GlobalData, $translate,WishlistSvc) {

            var self = this;
            self.allOrdersLoaded = false;
            self.allWishListsLoaded = false;
            var modalInstance;
            ...
            $scope.showWishListFilter = $scope.showWishListsDefault;
            ...
            /*$scope.showAllWishListItems = function () {
                $scope.showAllWishListsButton = !$scope.showAllWishListsButton;

                var params = {
                    pageSize: GlobalData.wishlistItems.meta.total
                };
                if (self.allWishListsLoaded) {
                    $scope.showWishListFilter = $scope.showAllWishListsButton ? $scope.showWishListsDefault : $scope.wishlistItems.length;
                    $scope.showWishListsButtons = ($scope.wishlistItems.length > $scope.showWishListsDefault);

                } else {
                    WishlistSvc.query().then(function (items) {
                        $scope.wishlistItems = items;

                        // show filtered list or show all orders. Hide if all data is shown within filter.
                        $scope.showWishListFilter = $scope.showAllWishListsButton ? $scope.showWishListsDefault : $scope.wishlistItems.length;
                        $scope.showWishListsButtons = ($scope.wishlistItems.length > $scope.showWishListsDefault);
                        self.allWishListsLoaded = true;
                    });
                }
            };
            */
            ...
            $scope.computeTotalPrice = function () {
                WishlistSvc.computeTotalPrice();
            };
